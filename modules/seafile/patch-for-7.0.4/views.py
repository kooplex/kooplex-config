...
def oauth_callback(request):
    ...
    try:
        http_protocol = os.getenv('REWRITEPROTO', 'https')
        outerhost = os.getenv('OUTERHOST', 'localhost')
        token = session.fetch_token(
            TOKEN_URL,
            client_secret=CLIENT_SECRET,
#            authorization_response=request.get_full_path())
            authorization_response="%s://%s/%s"%(http_protocol, outerhost, request.get_full_path()))

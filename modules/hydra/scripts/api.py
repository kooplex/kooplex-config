...
CONF_DIR = os.getenv("HYDRACONF_DIR", "/etc/hydra/")
SECRET_DIR = os.getenv("SECRETS_DIR", "/etc/secrets/")
HYDRA_USER = os.getenv("HYDRA_API_USER", "hydra")
...
def create_client(service):
    try:
        ...
        assert not os.path.exists(client_file), f'File {client_file} exists'
        assert not os.path.exists(secret_file), f'File {secret_file} exists'
...
def create_policy(service):
    try:
        ...
        assert not os.path.exists(policy_file), f'File {policy_file} exists'
